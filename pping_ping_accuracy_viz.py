...
def parse_epping_rtt_line(line):
    ...
    if standard_format:
        ...
        time = util.parse_unix_timestamp(words[0])
        rtt = float(words[1])

    return {"timestamp": time, "rtt": rtt, "flow": flow,
            **parse_extra_epping_args(words)}
...
def parse_extra_epping_args(words):
    extra_args = dict()
    for word in words:
        if "=" in word:
            arg, val = word.split("=")
            extra_args[arg] = util.try_parse_numeric(val, fallback="str")
...
def read_rtt_data(data_folder, normalize_timestamps=True):
    data = dict()
    ...
    if normalize_timestamps:
        start = min(df["timestamp"].min() for df in data.values())
        for df in data.values():
            df["timestamp"] = util.normalize_timestamps(df["timestamp"], start)
...
def plot_rtt_diff(data1, data2, axes=None, unit="s", stats_label="Difference",
                  print_stats=True, stat_kwargs=None, **kwargs):
    if len(data1) != len(data2):
        raise ValueError("data1 and data2 of different size - cannot correlate")

    diff = (data1["rtt"].values - data2["rtt"].values) * unit_conversion[unit]
    diff_grouped_df = {stats_label: pd.DataFrame({"diff": diff})}
    ...
    complot.plot_pergroup_histogram(diff_grouped_df, col="diff", axes=axes,
                                    print_stats=print_stats,
                                    stat_kwargs=table_kwargs, **kwargs)
...
def plot_rtt_timeseries(data, normalize_timestamps=True, axes=None,
    ...
    if timestamp_type == "time":
        data_to_use = {key: df.copy() for key, df in data.items() if "timestamp" in df.columns}
        if normalize_timestamps:
...
def main():
    ...
    fig.savefig(os.path.join(args.input, "RTT_timeseries.png"),
                bbox_inches="tight")
    try:
        fig = plot_rtt_diff(data["ping"], data["ePPing"]).get_figure()
