...
def _is_humanreadable_timestamp(somestr):
    return re.match("^\d\d:\d\d:\d\d.\d+$", somestr) is not None


def _parse_humanreadable_timestamp(date, t_str):
    """
...
def parse_epping_message(line, date, src_ip=None):
    ...
    if not _is_humanreadable_timestamp(words[0].rstrip(":")):
        return None, None
    t = _parse_humanreadable_timestamp(date, words[0].rstrip(":"))
