...
def plot_ss_tcp_data(flow_dfs, title=None):
    fig, axes = plt.subplots(4, 1, figsize=(8, 8), constrained_layout=True,
                             gridspec_kw={"height_ratios": [0.1, 1, 0.1, 1]})
    axes[0].axis("off")
    plot_throughput_timeseries(flow_dfs, axes=axes[1])
    axes[2].axis("off")
    plot_rtt_timeseries(flow_dfs, axes=axes[3])
